...
class NormalizeTotalUsageByProductTypeAndDateJob implements ShouldQueue
{
     ...
     * @return void
     */
    public function handle(Reports $report_model, AwsBilling $billing, DailyProductUsageByType $usage_model)
    {
        ...
        if($amt_products > 0)
        {
            ...
            while($current_date_being_worked_on != $end_date)
            {
                ...
                {
                    BreakdownProductTypeProcessing::dispatch($this->report_id, $product, $current_date_being_worked_on)
